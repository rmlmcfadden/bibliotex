...
with open("bibliotex.tex", "w") as fh:
    ...
    fh.write(
        r"\renewcommand\showkeyslabelformat[1]{\fcolorbox{black}{yellow!50!white}{\tiny\texttt{#1}}}"
        + "\n"
    )
    ...
    fh.write(
        r"\date{\textit{Last updated}: " + t.strftime("%Y-%m-%d %H:%M:%S") + "}" + "\n"
    )
    fh.write("\n")
    ...
    for b in bib:
        ...
        fh.write(
            "\t"
            + r"\printbibliography[title = {"
            + b.replace("references/", "", 1).replace(current_dir, "", 1)
            + "}, heading = subbibintoc]"
            + "\n"
        )
